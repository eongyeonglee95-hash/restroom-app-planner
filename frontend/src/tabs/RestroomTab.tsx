...
type FilterKey = '전체' | '공공화장실' | '편의점' | '카페'
const FILTERS: FilterKey[] = ['전체', '공공화장실', '편의점', '카페']
  ...
  return (
    <div className="relative h-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-x-3 top-3 z-20 flex flex-col gap-2">
        ...
        <div className="pointer-events-auto flex w-full items-center gap-1 overflow-x-auto py-1 lg:w-[390px]">
          {FILTERS.map((f) => (
            <button
              key={f}
              type="button"
              ...
              className={
                filter === f
                  ? 'shrink-0 rounded-full bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white shadow-md shadow-blue-600/30'
                  : 'shrink-0 rounded-full bg-white/40 px-3 py-1.5 text-xs font-semibold text-slate-600 shadow-sm shadow-black/5 backdrop-blur-md'
              }
            >
          ...
          <button
            ...
            className={
              withKid
                ? 'flex shrink-0 items-center gap-1 rounded-full bg-emerald-500 px-3 py-1.5 text-xs font-semibold text-white shadow-md shadow-emerald-500/30'
                : 'flex shrink-0 items-center gap-1 rounded-full bg-white/40 px-3 py-1.5 text-xs font-semibold text-slate-600 shadow-sm shadow-black/5 backdrop-blur-md'
            }
          >
            👶 아이와 함께
          </button>
      ...
      {/* 리스트 패널: 스크롤되는 순위 리스트만 (상세 카드는 하단 footer로 분리) */}
      <div
        className={`absolute inset-x-0 bottom-0 z-20 flex max-h-[85%] flex-col overflow-hidden rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.18)] backdrop-blur-md transition-colors duration-200 hover:bg-white/95 lg:inset-x-auto lg:bottom-auto lg:left-3 lg:top-28 lg:max-h-[54rem] lg:w-[390px] lg:rounded-3xl ${
      ...
      >
        ...
        <div className="max-h-[778px] overflow-y-auto">
      ...
      {/* 선택된 카드 상세: 리스트 클릭 시 펼쳐짐. 화면 하단에, footer 전체 폭이 아니라 리스트 패널과 같은 폭으로 떠 있는 카드 */}
      {selected && detailExpanded && (
        <div className="absolute inset-x-3 bottom-3 z-30 h-[85vh] overflow-hidden rounded-3xl bg-white shadow-[0_8px_30px_rgba(0,0,0,0.18)] lg:inset-x-auto lg:left-3 lg:h-[26rem] lg:w-[390px]">
          <button
            type="button"
