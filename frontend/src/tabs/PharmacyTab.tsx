...
import PharmacyDetailCard from '../components/PharmacyDetailCard'
import { businessHoursOf, fetchNearbyPharmacies, formatWalkingTime, type PharmacyPlace } from '../lib/api'
import { TIER_CLASS, tierOf } from '../lib/tier'

type FilterKey = '전체' | '야간'
const FILTERS: FilterKey[] = ['전체', '야간']
...
function PharmacyTab() {
  ...
  const [places, setPlaces] = useState<PharmacyPlace[] | null>(null)
  ...
  useEffect(() => {
    ...
    const layer = filter === '전체' ? 'GENERAL' : 'NIGHT'
    fetchNearbyPharmacies(markerPosition.lat, markerPosition.lng, layer, openNowOnly)
      .then((data) => {
        if (cancelled) return
        setPlaces(data)
      })
  ...
  }, [markerPosition, filter, openNowOnly])
  ...
  useEffect(() => {
    ...
    mapRef.current?.animateWalk('pharmacy')
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected?.id])
  ...
  return (
    <div className="relative h-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-x-3 top-3 z-20 flex flex-col gap-2">
        ...
        <div className="pointer-events-auto -mx-3 flex w-[calc(100%+1.5rem)] items-center gap-2 px-3 py-1.5 lg:mx-0 lg:w-[390px] lg:overflow-x-auto lg:px-0">
          {FILTERS.map((f) => (
            <button
              key={f}
              type="button"
              ...
              className={
                filter === f
                  ? 'flex-1 whitespace-nowrap rounded-full bg-cham-purple px-3.5 py-2 text-center text-xs font-semibold text-white shadow-md shadow-cham-purple/30 lg:flex-none'
                  : 'flex-1 whitespace-nowrap rounded-full bg-white/50 px-3.5 py-2 text-center text-xs font-semibold text-cham-ink/70 shadow-sm shadow-black/5 backdrop-blur-md lg:flex-none'
              }
            >
              {FILTER_ICON[f]} {f}
            </button>
          ...
          <button
            ...
            className={
              openNowOnly
                ? 'flex flex-1 items-center justify-center gap-1 whitespace-nowrap rounded-full bg-emerald-500 px-3.5 py-2 text-xs font-semibold text-white shadow-md shadow-emerald-500/30 lg:flex-none'
                : 'flex flex-1 items-center justify-center gap-1 whitespace-nowrap rounded-full bg-white/50 px-3.5 py-2 text-xs font-semibold text-cham-ink/70 shadow-sm shadow-black/5 backdrop-blur-md lg:flex-none'
            }
          >
            ✅ 지금 영업중
          </button>
        ...
        >
          ...
          <div className="flex shrink-0 items-center justify-between px-4 pb-1 pt-2">
            <h2 className="text-sm tracking-tight">
              <span className="font-bold text-cham-ink">가까운 약국</span>{' '}
              <span className="font-light text-cham-ink/45">
                {loading ? '검색 중...' : `${list.length}곳`}
              </span>
            </h2>
          </div>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <span className="truncate text-sm font-bold tracking-tight text-cham-ink">{place.name}</span>
                        ...
                        <div className="mt-1 flex items-baseline gap-1.5">
                          ...
                          {hours ? (
                            <span className="font-mono-time tracking-tight">
                              <span className="text-xs font-medium text-cham-ink/50">{hours.open}~</span>
                              <span className="text-sm font-bold text-cham-ink">{hours.close}</span>
                              <span className="ml-0.5 text-[11px] font-semibold text-cham-ink/60">마감</span>
                            </span>
                          ) : (
                            <span className="text-[11px] font-light text-cham-ink/45">{place.todayHours}</span>
                          )}
                        </div>
                      </div>
        ...
        {selected && detailExpanded && (
          <div className="absolute inset-x-0 bottom-0 z-20 h-60 overflow-hidden rounded-t-3xl bg-white shadow-[0_-8px_30px_rgba(0,0,0,0.18)] lg:relative lg:inset-auto lg:z-auto lg:mt-2 lg:h-auto lg:min-h-[24rem] lg:flex-1 lg:w-max lg:max-w-[640px] lg:min-w-full lg:self-start lg:rounded-3xl lg:shadow-[0_8px_30px_rgba(0,0,0,0.18)]">
            ...
            <div className="h-full p-1.5">
              <PharmacyDetailCard place={selected} origin={markerPosition} />
            </div>
          </div>
        )}
      </div>
